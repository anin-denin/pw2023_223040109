...
if(isset($_POST['submit'])){ // Memeriksa apakah tombol 'submit' telah diklik.
    ...
    $address = $_POST['flat'] .', '.$_POST['building'].', '.$_POST['area'].', '.$_POST['town'] .', '. $_POST['city'] .', '. $_POST['state'] .', '. $_POST['country'] .' - '. $_POST['pin_code']; // Menggabungkan nilai dari input-form menjadi satu string alamat.
    $address = filter_var($address, FILTER_SANITIZE_FULL_SPECIAL_CHARS ); // Membersihkan nilai alamat dari karakter yang tidak diinginkan.

    $update_address = $conn->prepare("UPDATE `users` set address = ? WHERE id = ?"); // Mempersiapkan pernyataan SQL untuk memperbarui kolom 'address' pada tabel 'users' berdasarkan 'id' pengguna.
    $update_address->execute([$address, $user_id]); // Menjalankan pernyataan SQL dengan mengikat nilai alamat dan 'user_id'.

    $message[] = 'address saved!'; // Menyimpan pesan ke dalam array $message.
